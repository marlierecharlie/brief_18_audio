...
def compute_features(sig_t, sig_s, sig_c, sr=51200):
    ...
    t = np.linspace(0, (sig_t.shape[0] - 1)*(1/sr), sig_t.shape[0])
    ...
    freqs = np.linspace(0, sr/2, sig_s.shape[0])
    ...
    t_c = np.linspace(0, (sig_c.shape[0] - 1)*(1/sr), sig_c.shape[0])
